...
const MOCK_ASSETS = [
  { id: 'a1', code: 'MTR-01', name: 'Ana Konveyör Motoru', location: 'Hat 1', status: 'RUNNING', criticality: 'HIGH', healthScore: 88 },
  { id: 'a2', code: 'CNC-04', name: 'CNC Torna Tezgahı', location: 'İmalat', status: 'FAULT', criticality: 'HIGH', healthScore: 45 },
  { id: 'a3', code: 'PMP-12', name: 'Soğutma Pompası', location: 'Tesisat', status: 'MAINTENANCE', criticality: 'MEDIUM', healthScore: 60 },
  { id: 'a4', code: 'RBT-02', name: 'Kaynak Robotu', location: 'Hat 2', status: 'RUNNING', criticality: 'HIGH', healthScore: 95 },
];
...
export default function MaintenanceApp() {
  ...
  const [workOrders, setWorkOrders] = useState(MOCK_WOS);
        ...
        <div className="space-y-4 pt-6">
          ...
          {MOCK_USERS.map(user => (
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3 bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 rounded-xl transition-all group"
            >
  ...
  return (
    <>
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-[#141414] border-t border-neutral-800 px-6 py-2 flex items-center justify-between z-50">
        ...
        <button className="flex flex-col items-center p-2 text-neutral-500">
          <Menu size={20} />
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        ...
        <button 
          onClick={() => navigate('REPORT_FAILURE')}
          className="hidden md:flex bg-amber-500 hover:bg-amber-400 text-black px-4 py-2 rounded-lg items-center font-medium transition-colors"
        >
...
function TechnicianDashboard() {
  ...
  const myOrders = workOrders.filter((wo) => wo.status !== 'CLOSED');

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard title="Açık İşlerim" value={myOrders.length} icon={Wrench} color="blue" />
        <StatCard title="Kritik Arızalar" value={myOrders.filter((w) => w.priority === 'CRITICAL').length} icon={AlertTriangle} color="red" />
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard title="Açık İşler" value="12" icon={Wrench} color="amber" />
...
function WorkOrderCard({ wo, onClick }) {
  ...
  const statusMap = {
    NEW: 'YENİ', IN_PROGRESS: 'DEVAM EDİYOR', COMPLETED: 'TAMAMLANDI', CLOSED: 'KAPALI'
  };

  return (
    <div 
      onClick={onClick}
      className="bg-[#141414] p-4 rounded-xl border border-neutral-800 hover:border-neutral-600 transition-colors cursor-pointer group flex flex-col sm:flex-row gap-4"
    >
      <div className="flex-1">
        <div className="flex items-center space-x-2 mb-2">
          <span className="text-xs font-mono text-neutral-500">{wo.id}</span>
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border uppercase tracking-wider ${priorityColors[wo.priority]}`}>
            {wo.priority}
          </span>
  ...
  return (
    <div className="max-w-2xl mx-auto animate-in fade-in zoom-in-95 duration-300">
      <div className="flex items-center space-x-3 mb-6">
        <button onClick={() => navigate('DASHBOARD')} className="p-2 bg-neutral-900 rounded-lg hover:bg-neutral-800">
          <X size={20} />
        </button>
      ...
      <form onSubmit={handleSubmit} className="bg-[#141414] border border-neutral-800 rounded-2xl p-5 md:p-6 space-y-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <select 
              required
              className="w-full bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-3 text-neutral-200 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500"
              ...
              {['LOW', 'MEDIUM', 'CRITICAL'].map((level) => (
                <button
                  ...
                  className={`py-2 px-3 rounded-xl border text-sm font-semibold transition-all ${
                    form.priority === level 
                      ? level === 'CRITICAL' ? 'bg-red-500 text-white border-red-500' : 'bg-amber-500 text-black border-amber-500'
          ...
          <div>
            ...
            <input 
              required
              type="text" 
              placeholder="Örn: Konveyör motoru aşırı ısınıyor"
              className="w-full bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-3 text-neutral-200 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500"
          ...
          <div>
            <label className="block text-sm font-medium text-neutral-400 mb-1.5">Fotoğraf / Medya (Opsiyonel)</label>
            <div className="border-2 border-dashed border-neutral-800 rounded-xl p-8 flex flex-col items-center justify-center text-neutral-500 hover:bg-neutral-900/50 hover:border-neutral-700 transition-colors cursor-pointer">
              <Camera size={32} className="mb-2" />
              <p className="text-sm">Fotoğraf çekmek için tıklayın</p>
            </div>
          </div>
        </div>
        ...
        <button 
          type="submit" 
          disabled={loading}
          className="w-full bg-amber-500 hover:bg-amber-400 text-black font-bold text-lg py-4 rounded-xl flex items-center justify-center transition-colors disabled:opacity-70"
        >
...
function WorkOrderDetail() {
  const { selectedWO, setWorkOrders, assets, navigate } = React.useContext(AppContext);
  const wo = selectedWO;
  ...
  useEffect(() => {
    ...
    if (wo && wo.isTimerRunning) {
      interval = setInterval(() => {
        setWorkOrders((prev) => 
          prev.map(w => w.id === wo.id ? { ...w, timerSeconds: w.timerSeconds + 1 } : w)
        );
      }, 1000);
    }
    return () => {
  ...
  if (!wo) {
    return (
      <div className="flex items-center justify-center h-64 text-neutral-500 flex-col">
        <Activity className="animate-spin mb-4 text-amber-500" size={32} />
        <p>İş emri yükleniyor veya bulunamadı...</p>
      </div>
    );
  }
  ...
  const toggleTimer = () => {
    setWorkOrders((prev) => 
      prev.map(w => w.id === wo.id ? { 
        ...w, 
        isTimerRunning: !w.isTimerRunning,
        status: !w.isTimerRunning ? 'IN_PROGRESS' : w.status
      } : w)
    );
  };

  const completeWork = () => {
    if(window.confirm("İş emrini tamamlamak istediğinize emin misiniz?")) {
      setWorkOrders((prev) => 
        prev.map(w => w.id === wo.id ? { ...w, isTimerRunning: false, status: 'COMPLETED' } : w)
      );
      navigate('DASHBOARD');
    }
  };

  const loadAiDiagnostics = () => {
    setIsAiLoading(true);
    setTimeout(() => {
      setAiSuggestions([
        "Geçmiş kayıtlara göre bu motorda rulman ısınması sık görülüyor.",
        "Soğutma fanı çalışıyor mu kontrol edin.",
        "Faz akımlarını pens ampermetre ile ölçün."
      ]);
  ...
  return (
    <div className="h-full flex flex-col pb-24 md:pb-0 animate-in slide-in-from-right-8 duration-300">
      <div className="flex items-center space-x-3 mb-6">
        <button onClick={() => navigate('WORK_ORDERS')} className="p-2 bg-neutral-900 rounded-lg hover:bg-neutral-800">
          <ArrowRight size={20} className="rotate-180" />
        </button>
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-white flex items-center">
            {wo.id}
            {wo.status === 'COMPLETED' && <CheckCircle className="ml-2 text-emerald-500" size={20}/>}
          </h1>
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex items-start space-x-4">
              ...
              <div>
                ...
                <div className="mt-2 flex space-x-2">
                  <span className="text-xs px-2 py-1 bg-neutral-800 rounded text-neutral-300">{asset?.location}</span>
                  <button className="text-xs px-2 py-1 bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded hover:bg-amber-500/20">Geçmişi Gör</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#141414] border border-neutral-800 rounded-xl p-5">
            ...
            {aiSuggestions.length === 0 ? (
              <button 
                ...
                className="w-full py-3 border border-dashed border-amber-500/50 rounded-xl text-amber-500 hover:bg-amber-500/10 flex items-center justify-center font-medium transition-colors"
              >
              ...
              <div className="space-y-3 bg-amber-500/5 border border-amber-500/20 p-4 rounded-xl">
                <div className="flex items-center text-amber-500 mb-2 font-medium">
                  <Zap size={16} className="mr-2" /> Olası Nedenler & Kontroller
                </div>
          ...
          <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-5">
             <div className="flex items-center text-red-500 font-bold mb-3">
               <ShieldAlert size={20} className="mr-2" /> Güvenlik & LOTO
             </div>
          ...
          <div className="bg-[#141414] border border-neutral-800 rounded-xl p-6 text-center sticky top-24">
            ...
            {wo.status !== 'COMPLETED' ? (
              <div className="space-y-3">
                ...
                >
                  {wo.isTimerRunning ? <Pause className="mr-2" /> : <Play className="mr-2" />}
                  {wo.isTimerRunning ? 'MOLA VER' : 'ÇALIŞMAYA BAŞLA'}
                </button>
                
                <button 
                  onClick={completeWork}
                  disabled={wo.timerSeconds === 0}
                  className="w-full py-3 rounded-xl font-semibold bg-neutral-900 border border-neutral-700 text-neutral-300 hover:bg-neutral-800 disabled:opacity-50 transition-colors"
                >
                  İŞİ TAMAMLA
                </button>
              </div>
            ) : (
              <div className="p-4 bg-emerald-500/10 text-emerald-500 rounded-xl border border-emerald-500/20 font-bold flex items-center justify-center">
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {assets.map((asset) => (
          <div key={asset.id} className="bg-[#141414] border border-neutral-800 p-5 rounded-xl hover:border-amber-500/50 transition-colors cursor-pointer group">
            <div className="flex justify-between items-start mb-3">
              <span className="px-2 py-1 bg-neutral-900 rounded text-xs font-mono text-neutral-400">{asset.code}</span>
              <div className={`w-2.5 h-2.5 rounded-full ${
                asset.status === 'RUNNING' ? 'bg-emerald-500' :
                asset.status === 'FAULT' ? 'bg-red-500' : 'bg-amber-500'
